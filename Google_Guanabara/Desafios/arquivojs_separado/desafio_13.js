function media_final(){
    ...
    let name = String(window.prompt('Qual é o nome do aluno?'));
    let nota_1 = parseFloat(window.prompt(`Primeira nota de ${name}:`));
    let nota_2 = parseFloat(window.prompt(`Segunda nota de ${name}:`));
    let media_final = (nota_1 + nota_2) / 2;
    //blocos de condição
    if(media_final > 6){
        demo.innerHTML = `<h2>Analisando a situação de ${name}</h2>` +
